...
program
  .command('auth-local')
  .description('start local auth client')
  .option('-p, --port <port>'
    , 'port (or $PORT)'
    , Number
    , process.env.PORT || 7120)
...
program
  .command('local-admin')
  .description('Add/Replaced admin user')
  ...
  .action(function(options) {
    if (!options.username) {
      this.missingArgument('--username')
    }
    if (!options.email) {
      this.missingArgument('--email')
    }

    if (!options.password) {
      this.missingArgument('--password')
    }
...
program
  ...
  .option('--bind-dev-pull <endpoint>'
    ...
    , String
    , 'tcp://127.0.0.1:7116')
  .option('--auth-type <mock|local|ldap|oauth2>'
    , 'auth type'
    , String
    , 'local')
  .option('-a, --auth-url <url>'
